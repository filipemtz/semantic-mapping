...
filter_pointcloud(PointCloud<PointXYZRGB>::Ptr raw_cloud)
{
	...
	{
		if ((fabs(raw_cloud->at(i).x) > 5.0 || fabs(raw_cloud->at(i).y) > 2.0) && raw_cloud->at(i).x < 70.0 && raw_cloud->at(i).z > -1.5
...
void read_odom_data(char *filename, vector<pair<int, int>> &indices,
...
void increase_bightness(PointCloud<PointXYZRGB>::Ptr aligned)
{
...
void redraw_cloud(PointCloud<PointXYZRGB>::Ptr source, pcl::visualization::PCLVisualizer *viewer, Matrix<double, 4, 4> pose)
{
...
void update_file(char *filename, vector<Pose2d> &corrected_poses, vector<pair<int, int>> &indices, vector<int> &convergence_vector)
{
...
int main(int argc, char **argv)
{
	...
	while (1)
	{
		...
		while (1)
		{
			...
			if (c == 'e')
			{
				corrected_pose.th = normalize_theta(corrected_pose.th - degrees_to_radians(0.5));
				updated = 1;
			}
			if (c == 'q')
			{
