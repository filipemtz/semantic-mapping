...
#include "libsegmap/libcarmen_global/carmen.h"
#include "libsegmap/libcarmen_global/global.h"
#include <iostream>
#include <string>
#include <vector>
#include <cmath>
...
void read_loop_restrictions(string &filename, vector<LoopRestriction> *loop_data, int filter = 0)
{
	int n;
	FILE *f;
	double x, y, theta;
...
void add_vertices(vector<SE2> &dead_reckoning, SparseOptimizer *optimizer)
{
	uint i;
...
void add_odometry_edges(SparseOptimizer *optimizer, vector<SE2> &dead_reckoning, double xy_std, double th_std)
{
...
void add_xsens_edges(SparseOptimizer *optimizer, GraphSlamData &data)
{
...
void get_gps_angle(GraphSlamData &data, int i,
				   double th_std,
				   double *angle,
...
void add_gps_edges(GraphSlamData &data, SparseOptimizer *optimizer,
...
void add_gps_from_map_registration_edges(GraphSlamData &data, vector<LoopRestriction> &gicp_gps,
...
void add_loop_closure_edges(vector<LoopRestriction> &loop_data, SparseOptimizer *optimizer, double xy_std, double th_std)
{
...
void create_dead_reckoning(GraphSlamData &data, vector<SE2> &dead_reckoning)
{
	double dt;
	DataSample *sample;
...
void create_graph_using_log_data(GraphSlamData &data, SparseOptimizer *optimizer, int gps_step)
{
...
void save_corrected_vertices(GraphSlamData &data, SparseOptimizer *optimizer)
{
	double x, y, th;
	DataSample *sample;
...
void initialize_g2o_stuff(g2o::Factory *factory, SparseOptimizer *optimizer)
{
	shared_ptr<AbstractHyperGraphElementCreator> gps_edge_creator(new HyperGraphElementCreator<EdgeGPS>);
	factory->registerType("EDGE_GPS", gps_edge_creator);

	auto chomoldSolver = std::make_unique<MyCholmodSolver>();
	chomoldSolver->setBlockOrdering(false);

	// g2o::OptimizationAlgorithm *optimization_algorithm =
	//	new g2o::OptimizationAlgorithmGaussNewton(solver);
	OptimizationAlgorithmGaussNewton *gaussNewtonsolver =
		new OptimizationAlgorithmGaussNewton(
			std::make_unique<MyBlockSolver>(std::move(chomoldSolver)));

	optimizer->setAlgorithm(gaussNewtonsolver);
...
void free_g2o_stuff(g2o::Factory *factory,
...
void parse_command_line(int argc, char **argv, CommandLineArguments &args, GraphSlamData *data)
{
...
void add_graphslam_parameters(CommandLineArguments &args)
{
