...
#include "libsegmap/libcarmen_global/Gdc_Coord_3d.h"
#include "libsegmap/libcarmen_global/Utm_Coord_3d.h"
#include "libsegmap/libcarmen_global/Gdc_To_Utm_Converter.h"
...
int NewCarmenDataset::size() const
{
	return _data.size();
}
...
void NewCarmenDataset::_load_odometry_calibration(std::string &path)
{
	int success = false;
...
void NewCarmenDataset::_load_poses(std::string &path, std::vector<Pose2d> *poses, std::vector<double> *poses_times)
{
	bool success = false;
	char dummy[64];
...
void NewCarmenDataset::_update_data_with_poses()
{
...
void NewCarmenDataset::_load_log(std::string &path)
{
...
void NewCarmenDataset::_load_synchronizing_by_nearest(FILE *fptr)
{
...
void NewCarmenDataset::_load_synchronizing_by_nearest_before(FILE *fptr)
{
	double time;
...
void NewCarmenDataset::_synchronize_messages_by_times(vector<double> &reference_sensor_times)
{
	DataSample *sample;
...
void NewCarmenDataset::_read_and_enqueue_one_message(FILE *fptr)
{
...
void NewCarmenDataset::_clear_synchronization_queues()
{
	_imu_messages.clear();
...
void NewCarmenDataset::_add_message_to_queue(string line)
{
	// ignore small lines
	if (line.size() > 10)
	{
...
void NewCarmenDataset::_parse_odom(vector<string> data, DataSample *sample)
{
	assert(data.size() == 6);
...
void NewCarmenDataset::_parse_imu(vector<string> data, DataSample *sample)
{
	assert(data.size() == 19);
...
void NewCarmenDataset::_parse_velodyne(vector<string> data, DataSample *sample, string velodyne_path)
{
	assert(data.size() == 6);
...
void NewCarmenDataset::_parse_camera(vector<string> data, DataSample *sample, string image_path)
{
	assert(data.size() == 9);
...
void NewCarmenDataset::_parse_gps_position(vector<string> data, DataSample *sample)
{
...
void NewCarmenDataset::_parse_gps_orientation(vector<string> data, DataSample *sample)
{
