...
#define TRANSLATE3D(p, offset) \
	p.x += offset.x;           \
	p.y += offset.y;           \
	p.z += offset.z;

// TODO Verificar se esta funcionando
#define ROTATE3DZ(p, theta)                    \
	p.x = p.x * cos(theta) - p.y * sin(theta); \
	p.y = p.x * sin(theta) + p.y * cos(theta);

#define DISPLACE2D(p, offset, theta) \
	p.x += offset * cos(theta);      \
