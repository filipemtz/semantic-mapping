...
def run_mapper(args):
    log_name, intensity_mode, use_lane_segmentation = args
    mapper_args = " -v 1 "
    ...
    mapper_cmd = "time ./programs/mapper /home/filipe/data/logs_iara/%s %s --v_thresh 1 -i %s -m  %s %s" % (
        log_name, PARAM_FILE, intensity_mode, map_path, mapper_args)
    run_command(mapper_cmd)

    map_img_path = map_path + "_complete.png"
    create_complete_map_cmd = 'python scripts/visualization_and_analysis/generate_complete_map.py %s %s' % (
...
if __name__ == "__main__":
    ...
    for e in experiments:
        ...
        logs_to_map.append([e['map'], 'colour', False])
        logs_to_map.append([e['map'], 'reflectivity', False])
